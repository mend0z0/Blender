...
__IO bool fmpi2cReloadFlag = false;
__IO bool fmpi2cStopFlag = false;

void FMPI2C1_EV_IRQHandler( void )
{
  if((FMPI2C1->ISR & FMPI2C_ISR_NACKF) == FMPI2C_ISR_NACKF)	// (1)
    {
      FMPI2C1->ICR |= FMPI2C_ICR_NACKCF;			// (2)
    }
  else if((FMPI2C1->ISR & FMPI2C_ISR_TXIS) == FMPI2C_ISR_TXIS)	// (4)
    {
      if((FMPI2C1->CR1 & FMPI2C_CR1_TXDMAEN) == 0)	//if dma is already started that means we are in the middle of transfer
	{
	...
	{
	  if(fmp12cMaxNumberOfTransfer == 0)
	    {
	      FMPI2C1->CR2 |= FMPI2C_CR2_STOP;	//this part is for the left over data to be sent.
	    }
	  else
	    {
	      fmp12cMaxNumberOfTransfer--;
	    }
	}
    }
  else if((FMPI2C1->ISR & FMPI2C_ISR_TCR) == FMPI2C_ISR_TCR)
    {
      fmpi2cReloadFlag = true;
    }
  else if((FMPI2C1->ISR & FMPI2C_ISR_STOPF) == FMPI2C_ISR_STOPF)
    {
...
static void _init_FMPI2C( void )
{
  ...
  FMPI2C1->CR1 |= (	FMPI2C_CR1_TCIE 	| 	// Transfer Complete interrupt enabled
			...
			FMPI2C_CR1_STOPIE	|	// Stop detection (STOPF) interrupt enabled
...
int32_t FMPI2C1DataTx( uint8_t slaveAddr, uint8_t *data, uint32_t buffSize)
{
  ...
  if(buffSize <= FMPI2C_BUFF_SIZE)
      ...
      for(buffCnt = 0; buffCnt < buffSize; ++buffCnt)
	{
      ...
      FMPI2C1->CR2 |= (buffSize << FMPI2C_CR2_NBYTES_Pos) | 	// The number of bytes to be transmitted/received is programmed there. Note: Changing these bits when the START bit is set is not allowed.
		      FMPI2C_CR2_AUTOEND			// Automatic end mode: a STOP condition is automatically sent when NBYTES data are transferred.
		      ;
    }
  else
    {
      ...
      fmp12cMaxNumberOfTransfer = buffSize - FMPI2C_BUFF_SIZE;		// we'll deduct number of bytes that we put into the buffer.
    }
  ...
  while(1)
    {
      if(fmpi2cStopFlag == true)
	{
	  fmpi2cStopFlag = false;
	  break;
	}
      if(fmpi2cReloadFlag == true)
	{
	  if(fmp12cMaxNumberOfTransfer > FMPI2C_BUFF_SIZE)
	    {
	      for(buffCnt = 0; buffCnt < FMPI2C_BUFF_SIZE; ++buffCnt)
	      	{
	      	  fmp12cBuffer[cnt] = *data++;				// upload the pointed data to buffer and increase one address this way we use that variable as our counter as well
	      	}
	      fmp12cMaxNumberOfTransfer -= FMPI2C_BUFF_SIZE;		// we'll deduct number of bytes that we put into the buffer.
	    }
	  else
	    {
	      FMPI2C1->CR2 &= ~FMPI2C_CR2_RELOAD;			// The transfer is completed after the NBYTES data transfer (STOP or RESTART follows).
	      for(buffCnt = 0; buffCnt < fmp12cMaxNumberOfTransfer; ++buffCnt)
	      	{
	      	  fmp12cBuffer[cnt] = *data++;				// upload the pointed data to buffer and increase one address this way we use that variable as our counter as well
	      	}
	      FMPI2C1->CR1 |= FMPI2C_CR1_TXIE;				// TX Interrupt enable
	    }
	  fmpi2cReloadFlag = false;
	}
    }
